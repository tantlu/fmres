...
import { X, Save, Edit, Plus, Coffee, Link as LinkIcon } from 'lucide-react';
...
export default function AdminModal({ isOpen, onClose, initialData, onSave }: { isOpen: boolean; onClose: () => void; initialData?: ResourceItem | null; onSave: (data: ResourceItem) => void; }) {
  const [formData, setFormData] = useState<ResourceItem>({
    title: '', category: 'Face', author: '', image: '', downloadLink: '', description: '', views: 0, likes: 0, date: new Date().toISOString().split('T')[0],
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div className="bg-[#1e293b] rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-600">
        ...
        <div className="p-6 space-y-6">
          {/* Các trường nhập liệu cơ bản (Giữ nguyên) */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-400 mb-1 uppercase">Tiêu đề</label>
                <input type="text" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} className="w-full p-3 bg-[#0f172a] border border-slate-600 rounded-lg text-white focus:border-emerald-500 outline-none" placeholder="VD: DF11 Megapack" />
              </div>
          ...
          <div>
             <label className="block text-xs font-bold text-slate-400 mb-2 uppercase">Nội dung</label>
             <RichTextEditor value={formData.description} onChange={(html) => setFormData({ ...formData, description: html })} />
          </div>

          {/* PHẦN CẤU HÌNH DONATE ĐÃ CẬP NHẬT */}
          <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
            ...
            <div className="space-y-3">
              {/* Thêm trường Link Donate riêng */}
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase flex items-center gap-1"><LinkIcon size={10}/> Link Donate (PlayerDuo, Wescan...)</label>
                <input
                  type="text"
                  value={formData.donateLink || ''}
                  onChange={e => setFormData({ ...formData, donateLink: e.target.value })}
                  className="w-full p-2 bg-[#0f172a] border border-slate-600 rounded text-sm text-blue-300 focus:border-blue-500 outline-none"
              ...
              <div className="relative flex py-1 items-center">
                 ...
                 <span className="flex-shrink-0 mx-2 text-[10px] text-slate-500 uppercase">Hoặc chuyển khoản</span>
                 <div className="flex-grow border-t border-slate-700"></div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <input
                  ...
                  className="w-full p-2 bg-[#0f172a] border border-slate-600 rounded text-sm text-white focus:border-emerald-500 outline-none"
